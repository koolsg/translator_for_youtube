    ...
    (typeof browser !== "undefined" ? browser : chrome).tabs.create(
        ...
        (newTab) => {
            ...
            if (newTab.id) {
                ...
                storage.set({ translatorTabId: newTab.id });
            }
        },
    );
}
...
async function reuseOrCreateTab(newUrl) {
    ...
    const { translatorTabId } = await storage.get("translatorTabId");

    if (translatorTabId) {
        try {
            // 저장된 ID로 탭 정보를 가져옵니다.
            const tab = await api.tabs.get(translatorTabId);
            // 탭이 존재하면 URL을 업데이트하고 활성화합니다.
            api.tabs.update(tab.id, { url: newUrl, active: true });
        } catch (error) {
...
api.runtime.onMessage.addListener((request) => {
    if (request.action === "showVideoId" && request.data) {
        ...
        if (videoId) {
            const url = new URL(api.runtime.getURL("translator_ui.html"));
...
api.tabs.onRemoved.addListener(async (tabId) => {
    ...
    if (tabId === translatorTabId) {
        ...
        storage.remove("translatorTabId");
